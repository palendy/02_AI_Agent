...
[검색 실패] → 채팅 히스토리 검색
    ↓
[히스토리 없음] → GitHub Issue 검색
    ↓
최종 답변 + GitHub Issue 제안
```

### 검색 우선순위
1. **벡터 스토어 검색** (문서 기반)
2. **채팅 히스토리 검색** (과거 대화)
3. **GitHub Issue 검색** (유사한 이슈)
4. **GitHub Issue 제안** (새 이슈 생성)
...
### 1. **검색 정확도**
- **관련성 임계값**: 0.6 (기존 0.3에서 개선)
- **유사도 검색**: 0.5 (기존 0.8에서 개선)
- **재시도 횟수**: 최대 3회

### 2. **응답 속도**
- **벡터 검색**: ~200ms
- **채팅 히스토리**: ~100ms
- **GitHub Issue 검색**: ~500ms
- **Cross-Encoder Re-ranking**: ~300ms

### 3. **저장 효율성**
- **자동 저장**: 만족스러운 답변만 저장
- **품질 필터링**: 0.5 이상 점수만 저장
- **중복 제거**: 동일한 질문-답변 쌍 중복 저장 방지
