...
try:
    # Use original agent for now due to prompt template issues in improved version
    from srs_generation_agent import SRSGenerationAgent
    print("🔧 원본 SRS Generation 에이전트 사용")
  ...
  python run.py --config production spec.txt # 프로덕션 설정 사용
    ...
    try:
        if args.config and CONFIG_AVAILABLE:
            ...
            # 개선된 에이전트 사용시 anti-hallucination 설정 적용
            if USING_IMPROVED:
                # Anti-hallucination을 위해 temperature를 0.0으로 강제 설정
                temperature = 0.0
                print(f"🛡️  Anti-hallucination을 위해 온도를 {temperature}로 설정")
                agent = SRSGenerationAgent(
    ...
    try:
        ...
        if result["success"]:
            ...
            # 개선된 에이전트의 추가 정보 표시
            if USING_IMPROVED and 'validation_summary' in result:
                validation = result['validation_summary']
                print(f"\n🛡️  검증 결과:")
                print(f"   - 검증된 요구사항: {validation.get('validated_count', 0)}개")
                print(f"   - 거부된 요구사항: {validation.get('rejected_count', 0)}개")
                print(f"   - 평균 신뢰도: {validation.get('avg_confidence', 0):.2f}")
                
                if validation.get('rejected_count', 0) > 0:
                    print(f"   - 주요 거부 이유: {', '.join(validation.get('rejection_reasons', []))}")
